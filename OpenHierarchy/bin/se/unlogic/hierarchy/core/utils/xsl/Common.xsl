...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template name="createTextField">
		...
		<xsl:param name="dataattributes" select="null"/>
		...
		<input type="{$type}">
			...
			<xsl:if test="$dataattributes != ''">
				<xsl:for-each select="$dataattributes//mapEntry">
	...
	<xsl:template name="createDropdown">
		...
		<xsl:param name="labelDelimiter" select="null"/>
		<xsl:param name="labelElementName2Parenthesis" select="null"/>
	...
	<xsl:template name="createTagsMultipleDropdown">
		<xsl:param name="id" select="null"/>
		<xsl:param name="name" select="null"/>
		<xsl:param name="class" select="null"/>
		<xsl:param name="title" select="null"/>
		...
		<select multiple="multiple">
			...
			<xsl:if test="$requestparameters/parameter[name=$name]">
				<xsl:for-each select="$requestparameters/parameter[name=$name]/value">					
	...
	<xsl:template name="createToggleDropdown">
		<xsl:param name="id" select="null"/>
		<xsl:param name="name" select="null"/>
		<xsl:param name="class" select="null"/>
		<xsl:param name="title" select="''"/>
		<xsl:param name="disabled" select="null"/>
		<xsl:param name="labelTrue" select="null"/>
		<xsl:param name="labelFalse" select="null"/>
		<xsl:param name="selectedValue" select="''" />
		<xsl:param name="width" select="null"/>
		<xsl:param name="requestparameters" select="requestparameters"/>
		<xsl:param name="addEmptyOption" select="null"/>
		...
		<select>
			...
			<option value="true">
				<xsl:choose>
					<xsl:when test="$requestparameters">
						<xsl:if test="$requestparameters/parameter[name=$name]/value = 'true'">
							<xsl:attribute name="selected"/>
						</xsl:if>
					</xsl:when>
					<xsl:otherwise>
						<xsl:if test="'true' = $selectedValue">
							<xsl:attribute name="selected" />
						</xsl:if>
					</xsl:otherwise>
				</xsl:choose>
				
				<xsl:value-of select="$labelTrue"/>
			</option>
			
			<option value="false">
				<xsl:choose>
					<xsl:when test="$requestparameters">
						<xsl:if test="$requestparameters/parameter[name=$name]/value = 'false'">
							<xsl:attribute name="selected"/>
						</xsl:if>
					</xsl:when>
					<xsl:otherwise>
						<xsl:if test="'false' = $selectedValue">
							<xsl:attribute name="selected" />
						</xsl:if>
					</xsl:otherwise>
				</xsl:choose>
				
				<xsl:value-of select="$labelFalse"/>
			</option>
		</select>
	...
	<xsl:template name="escapeForJavascript">
		<xsl:param name="text"/>
