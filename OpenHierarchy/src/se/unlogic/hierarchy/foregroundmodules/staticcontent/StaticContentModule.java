...
import se.unlogic.standardutils.io.CloseUtil;
...
public class StaticContentModule implements ForegroundModule {
	...
	private static final String UNDETECTED_FILE_ENCODING = "UNDETECTED";
	...
	@ModuleSetting
	@CheckboxSettingDescriptor(name="Enable javascript encoding detection", description="Controls if this modules should try and detect encoding of Javascript files and set the response encoding accordingly.")
	private boolean enableJsFileEncodingDetection = true;
	
	protected Properties globalContentLinks;
	
	private ConcurrentHashMap<URL, String> jsFileEncodingMap = new ConcurrentHashMap<URL, String>();
	...
	protected void checkInstanceHandlerRegistration(boolean register){
		...
		if(register){
			...
			systemInterface.getInstanceHandler().removeInstance(StaticContentModule.class, this);
		}
	}
	...
	private void sendFile(HttpServletRequest req, HttpServletResponse res, URIParser uriParser, URL url, InputStream fileStream, User user, String staticContentPackage, String filePath, ModuleDescriptor moduleDescriptor, String moduletype) {
		...
		try{
			...
			if(enableJsFileEncodingDetection && filePath.endsWith(".js")){
				
				String fileEncoding = jsFileEncodingMap.get(url);
				
				if(fileEncoding == null){
					
					fileEncoding = getFileEncoding(url, filePath);
					jsFileEncodingMap.put(url, fileEncoding);
				}
				
				if(fileEncoding != UNDETECTED_FILE_ENCODING){
					
					if(log.isDebugEnabled()){
						
						log.debug("Setting response encoding " + fileEncoding + " for file " + filePath);
					}
					
					res.setCharacterEncoding(fileEncoding);
				}
			}
	...
	private String getFileEncoding(URL url, String filePath) {
		...
		try{
		    ...
		    while ((bytesRead = inputStream.read(buf)) > 0 && !detector.isDone()) {
			...
			log.error("Error detecting file encoding of file " + filePath, t);
			
		}finally{
			
			CloseUtil.close(inputStream);
			
			//System.out.println("Scanned file " + filePath + " in " + TimeUtils.millisecondsToString(System.currentTimeMillis() - startTime));
		}
		
		return UNDETECTED_FILE_ENCODING;
	}
	...
	public String getModuleContentURL(ForegroundModuleDescriptor moduleDescriptor) {
		...
		return systemInterface.getContextPath() + this.sectionInterface.getSectionDescriptor().getFullAlias() + "/" + this.moduleDescriptor.getAlias() + "/f/" + moduleDescriptor.getSectionID() + "/" + moduleIdentifier;
	}
