...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:exsl="http://exslt.org/common">
	...
	<xsl:template match="ProviderConfiguration">
		...
		<div class="floatleft loginprovider bigmargintop bigmarginbottom full">
	...
	<xsl:template match="Configure">
		...
		<form method="POST" action="{/Document/requestinfo/uri}">
			...
			<div class="full margintop padding marginbottom">
				<strong>
					<xsl:value-of select="$i18n.Header"/>
				</strong>
				
				<xsl:apply-templates select="../../ValidationErrors/validationError[fieldName = 'header']"/>
			...
			<div class="full margintop padding marginbottom">
				<strong>
					<xsl:value-of select="$i18n.Footer"/>
				</strong>
				
				<xsl:apply-templates select="../../ValidationErrors/validationError[fieldName = 'footer']"/>
				
				<xsl:call-template name="createTextArea">
					<xsl:with-param name="name" select="'footer'"/>
					<xsl:with-param name="class" select="'htmleditor'"/>
