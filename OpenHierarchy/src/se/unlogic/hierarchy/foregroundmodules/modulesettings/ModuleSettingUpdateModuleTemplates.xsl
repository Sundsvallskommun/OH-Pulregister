...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="SelectSettingDescriptors">
		...
		<script>
			$(function () {
				$("#settingstable").on("change", "input[type=checkbox]", function () {
					$("#usei18n-" + $(this).val()).prop("checked", $(this).is(":checked"));
					$("#i18n-" + $(this).val()).toggle($(this).is(":checked"));
				});
			});
		</script>
		...
		<form method="POST" action="{/document/requestinfo/uri}">
			...
			<table id="settingstable" class="border full">
				<tr>
					<th></th>
			...
			<h1><xsl:value-of select="$i18n.settingsTranslations"/></h1>
			...
			<table class="border full">
				...
				<xsl:apply-templates select="SettingDescriptors/settingDescriptor" mode="i18n"/>
			</table>
	...
	<xsl:template match="settingDescriptor" mode="i18n">
	
		<xsl:variable name="id" select="id"/>
		<xsl:variable name="name">
			<xsl:choose>
				<xsl:when test="../../UpdateModuleSettings/AllowedSetting[id = $id]/i18nName">
					<xsl:value-of select="../../UpdateModuleSettings/AllowedSetting[id = $id]/i18nName"/>
				</xsl:when>
				<xsl:otherwise>
		...
		<xsl:variable name="description">
			<xsl:choose>
				<xsl:when test="../../UpdateModuleSettings/AllowedSetting[id = $id]/i18nDescription">
					<xsl:value-of select="../../UpdateModuleSettings/AllowedSetting[id = $id]/i18nDescription"/>
				</xsl:when>
				<xsl:otherwise>
		...
		<tr style="display: none" id="i18n-{id}">
			<xsl:if test="../../SelectedSettings/ID = $id">
				<xsl:attribute name="style"></xsl:attribute>
			</xsl:if>
			<td>
				<input type="checkbox" name="usei18n-{id}" id="usei18n-{id}">
					
					<xsl:if test="../../UpdateModuleSettings/AllowedSetting[id = $id]/i18nDescription or ../../UpdateModuleSettings/AllowedSetting[id = $id]/i18nName">
						<xsl:attribute name="checked">checked</xsl:attribute>
					</xsl:if>
			...
			<td><xsl:value-of select="id"/></td>
			<td><input type="text" name="i18nname-{id}" value="{$name}" class="full"/></td>
			<td><input type="text" name="i18ndescription-{id}" value="{$description}" class="full"/></td>
		</tr>
