...
public class ModuleUtils {
					...
					try {
						...
						if (Modifier.isFinal(field.getModifiers())) {
							...
							if (values != null || moduleSetting.allowsNull()) {
								...
								if(CollectionUtils.getSize(values) == 1 && values.get(0).contains("\n")){
									
									values = Arrays.asList(values.get(0).replace("\r", "").split("\\n"));
								}
			...
			for (Field field : fields) {
				...
				if (field.isAnnotationPresent(CheckboxSettingDescriptor.class)) {
					...
					if (field.getType() == boolean.class) {
						...
						defaultValue = (Boolean) field.get(moduleInstance);
					...
					settingDescriptors.add(SettingDescriptor.createMultiListSetting(id, annotation.name(), annotation.description(), annotation.required(), getSettingDescriptorDefaultListValues(field, moduleInstance), valueDescriptors));
	...
	private static List<String> getSettingDescriptorDefaultListValues(Field field, Module<?> moduleInstance) throws IllegalArgumentException, IllegalAccessException {
	...
	public static boolean checkRequiredModuleSettings(ModuleDescriptor moduleDescriptor, Module<?> moduleInstance, SystemInterface systemInterface, Level level) throws IllegalArgumentException, IllegalAccessException, InstantiationException, SQLException {

		boolean allSet = true;
		
		for (SettingDescriptor settingDescriptor : getAnnotatedSettingDescriptors(moduleInstance, null, systemInterface)) {
			...
			if (settingDescriptor.isRequired()) {
				...
				try {
					field = ReflectionUtils.getField(moduleInstance.getClass(), settingDescriptor.getId());

				} catch (RuntimeException ignoreCustomID) {}

				if (field != null) {

					ReflectionUtils.fixFieldAccess(field);

					if (field.get(moduleInstance) == null) {
						
						allSet = false;

						log.log(level, "Required setting " + settingDescriptor.getName() + " in module " + moduleDescriptor + " is not set");
					}
				}
			}
		}
