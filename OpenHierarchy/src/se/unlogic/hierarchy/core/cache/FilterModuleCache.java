...
public class FilterModuleCache extends WildcardModuleCache<FilterModuleDescriptor, FilterModule, FilterModuleCacheListener> {
	...
	public void cacheModules(BlockingQueue<Runnable> taskQueue) throws KeyNotCachedException, KeyAlreadyCachedException, SQLException {
		...
		try {
			// Get all enabled modules from the database
			List<? extends FilterModuleDescriptor> moduleDescriptors = moduleDAO.getEnabledModules();

			if (moduleDescriptors != null) {

				for (final FilterModuleDescriptor moduleDescriptor : moduleDescriptors) {

					if (!this.instanceMap.containsKey(moduleDescriptor)) {

						taskQueue.add(new ModuleStartupTask<FilterModuleDescriptor, FilterModuleCache>(moduleDescriptor, this, null));
					}
				}
			}
	...
	public void cacheModules(ThreadPoolTaskGroupHandler<TaskGroup> threadPoolTaskHandler) {

		LinkedBlockingQueue<Runnable> taskQueue = new LinkedBlockingQueue<Runnable>();

		try {
			cacheModules(taskQueue);
			
			ExecutionController<TaskGroup> startupExecutor = new SimpleExecutionController<TaskGroup>(new SimpleTaskGroup(taskQueue), threadPoolTaskHandler);

			try {
				startupExecutor.start();
				startupExecutor.awaitExecution();

			} catch (InterruptedException e) {
				
				log.error("Interrupted waiting for startup of filter modules", e);
			}			
