...
public class CoreXSLTCacheHandler extends XSLCacheHandler {
	...
	public CoreXSLTCacheHandler(SettingNode config, Language defaultLanguage, String applicationFileSystemPath){
		...
		for(SettingNode settingNode : stylesheets){
			...
			boolean isDefault = settingNode.getPrimitiveBoolean("@default");
			boolean fullPath = settingNode.getPrimitiveBoolean("@pathtype = 'Full'");
			boolean useFullMenu = settingNode.getPrimitiveBoolean("@fullMenu");
			...
			try {
				...
				if(fullPath){
					...
					cachedXSLTFile = new FileXSLTransformer(new File(path), ClassPathURIResolver.getInstance(), false);

				}else{

					cachedXSLTFile = new FileXSLTransformer(new File(applicationFileSystemPath + "WEB-INF/" + path), ClassPathURIResolver.getInstance(), false);
				}
