...
public class BackgroundModuleCache extends WildcardModuleCache<BackgroundModuleDescriptor, BackgroundModule, BackgroundModuleCacheListener> {
			...
			if (moduleDescriptorBeanList != null && moduleDescriptorBeanList.size() > 0) {
					...
					if (!this.instanceMap.isEmpty()) {
						...
						for (BackgroundModuleDescriptor moduleDescriptor : keys) {
							...
							if (!moduleDescriptorBeanList.contains(moduleDescriptor)) {
								...
								log.info("Background module " + moduleDescriptor + " not found in new module list for section " + this.sectionInterface.getSectionDescriptor() + ", removing from cache...");

								try {
									this.unload(moduleDescriptor);
								} catch (Throwable e) {
									log.error("Error unloading background module " + moduleDescriptor + " from background module cache in section "	+ this.sectionInterface.getSectionDescriptor(), e);
								}
							}
						}
				...
				for (BackgroundModuleDescriptor moduleDescriptor : moduleDescriptorBeanList) {
					if (this.instanceMap.containsKey(moduleDescriptor)) {
						...
						// Cache new module
						log.info("Adding background module " + moduleDescriptor + " to cache in section " + this.sectionInterface.getSectionDescriptor() + "...");
						try {
							this.cache(moduleDescriptor);
						} catch (Throwable e) {
							log.error("Error caching new instance of background module " + moduleDescriptor + " in section " + this.sectionInterface.getSectionDescriptor(), e);
						}
					}
				}
	...
	public void cacheModules(BlockingQueue<Runnable> taskQueue) throws KeyNotCachedException, KeyAlreadyCachedException, SQLException {
		...
		try {
			// Get all enabled modules from the database
			List<? extends BackgroundModuleDescriptor> moduleDescriptors = moduleDAO.getEnabledModules(this.sectionInterface.getSectionDescriptor().getSectionID());

			if (moduleDescriptors != null) {

				for (final BackgroundModuleDescriptor moduleDescriptor : moduleDescriptors) {

					if (!this.instanceMap.containsKey(moduleDescriptor)) {

						taskQueue.add(new ModuleStartupTask<BackgroundModuleDescriptor, BackgroundModuleCache>(moduleDescriptor, this, sectionInterface.getSectionDescriptor()));
					}
				}
			}
