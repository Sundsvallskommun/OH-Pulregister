	...
	private final static class UserSingleton implements Serializable {
		...
		private void removeUser(User user) {
			...
			try {
				...
				for(User logginInUser : loggedInUsers){
					
					if(user.equals(logginInUser)){
						
						try {
							if(logginInUser.getSession() != null && user.getSession() != null && logginInUser.getSession().getId().equals(user.getSession().getId())){
							...
							//Session got invalidated during request
						}
					}
