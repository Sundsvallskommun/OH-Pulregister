...
import se.unlogic.hierarchy.foregroundmodules.modulesettings.beans.AllowedSetting;
...
public class ModuleSettingUpdateModule extends AnnotatedForegroundModule implements AccessInterface {
	...
	protected List<AllowedSetting> allowedSettingBeans;
	...
	protected void moduleConfigured() {
		...
		allowedSettingBeans = new ArrayList<AllowedSetting>();

		if (allowedSettings != null) {
			for(String setting : allowedSettings){
				
				String i18nName = moduleDescriptor.getMutableSettingHandler().getString(setting + ".i18nName");
				String i18nDescription = moduleDescriptor.getMutableSettingHandler().getString(setting + ".i18nDescription");
	
				AllowedSetting allowedSetting = new AllowedSetting();
				allowedSetting.setId(setting);
				allowedSetting.setI18nName(i18nName);
				allowedSetting.setI18nDescription(i18nDescription);
				
				allowedSettingBeans.add(allowedSetting);
			}
		}
	}
	...
	public synchronized ForegroundModuleResponse defaultMethod(HttpServletRequest req, HttpServletResponse res, User user, URIParser uriParser) throws Exception {
		...
		XMLUtils.append(doc, listSettingsElement, "UpdateModuleSettings", allowedSettingBeans);
	...
	public synchronized ForegroundModuleResponse selectAllowedSettings(HttpServletRequest req, HttpServletResponse res, User user, URIParser uriParser) throws Exception {
		...
		if (req.getMethod().equalsIgnoreCase("POST")){
			...
			if (ids != null){
				...
				for(String setting : allowedSettings){
					if (!StringUtils.isEmpty(req.getParameter("usei18n-" + setting))){
						String name = req.getParameter("i18nname-" + setting);
						String description = req.getParameter("i18ndescription-" + setting);

						if (!StringUtils.isEmpty(name)){
							mutableSettingHandler.setSetting(setting + ".i18nName", name);
						}
						if (!StringUtils.isEmpty(description)){
							mutableSettingHandler.setSetting(setting + ".i18nDescription", description);
						}
					}else{
						mutableSettingHandler.removeSetting(setting + ".i18nName");
						mutableSettingHandler.removeSetting(setting + ".i18nDescription");
					}
				}
		...
		XMLUtils.append(doc, selectSettingDescriptorsElement, "UpdateModuleSettings", allowedSettingBeans);
	...
	public List<SettingDescriptor> getSettings() {
		...
		if (allowedSettings != null) {
			for(String setting : allowedSettings){
				...
				if (i18nName != null) {
					settings.add(SettingDescriptor.createTextFieldSetting(setting + ".i18nName", setting + ".i18nName", setting + ".i18nName", false, i18nName, null));
				}
				
				if (i18nDescription != null) {
					settings.add(SettingDescriptor.createTextFieldSetting(setting + ".i18nDescription", setting + ".i18nDescription", setting + ".i18nDescription", false, i18nDescription, null));
				}
			}
		}
