...
public class HTMLOutputModule extends AnnotatedBackgroundModule implements AccessInterface, SystemStartupListener, ForegroundModuleCacheListener {
	...
	public BackgroundModuleResponse processBackgroundRequest(HttpServletRequest req, User user, URIParser uriParser) throws Exception {
		...
		HTMLOutputAdminModule htmlOutputAdminModule = WeakReferenceUtils.getReferenceValue(this.htmlOutputAdminModuleRef);
		...
		if(htmlOutputAdminModule != null && AccessUtils.checkAccess(user,this) && AccessUtils.checkAccess(user,htmlOutputAdminModule.getModuleDescriptor())) {
			...
			this.htmlOutputAdminModuleRef = new WeakReference<HTMLOutputAdminModule>(htmlOutputAdminModule);

			XMLUtils.appendNewElement(doc, document, "settingsURL", htmlOutputAdminModule.getFullAlias() + "/settings/" + this.moduleDescriptor.getModuleID());
			XMLUtils.appendNewElement(doc, document, "deleteURL", htmlOutputAdminModule.getFullAlias() + "/delete/" + this.moduleDescriptor.getModuleID());
