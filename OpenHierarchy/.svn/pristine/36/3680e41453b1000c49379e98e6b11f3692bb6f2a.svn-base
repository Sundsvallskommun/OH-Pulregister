...
public class CoreServlet extends BaseServlet implements FullSystemInterface {
	...
	public static final String VERSION_PREFIX = "OpenHierarchy 1.2.10";
	...
	public void init(boolean throwExceptions) throws ServletException {
		...
		try {
			...
			if (!configurationFile.exists()) {
				...
				SettingNode startupThreading = config.getNode("/Config/StartupThreading");
				
				if(startupThreading != null && startupThreading.getPrimitiveBoolean("MultithreadStartup") && ManagementFactory.getRuntimeMXBean().getUptime() >= (startupThreading.getInt("RequiredUptime") * MillisecondTimeUnits.MINUTE)){
					
					Integer threads = startupThreading.getInteger("Threads");
					
					if(threads == null || threads < 1){
						
						threads = ManagementFactory.getOperatingSystemMXBean().getAvailableProcessors();
					}
					
					log.info("Using multithreaded startup with " + threads + " threads");
					
					ThreadPoolTaskGroupHandler<TaskGroup> threadPoolTaskHandler = new ThreadPoolTaskGroupHandler<TaskGroup>(threads, false);

					filterModuleCache.cacheModules(threadPoolTaskHandler);		
					
					rootSection.cacheModulesAndSections(threadPoolTaskHandler);

					threadPoolTaskHandler.shutdown();					
					...
					rootSection.cacheModulesAndSections();
				}
	...
	@Override
	public void addStartupListener(SystemStartupListener listener) {
	...
	public void destroy() {
		...
		try {
			...
			if(this.filterModuleCache != null){
				this.filterModuleCache.unload();
			}
