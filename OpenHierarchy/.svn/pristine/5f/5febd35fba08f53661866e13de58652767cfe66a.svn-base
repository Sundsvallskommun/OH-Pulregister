			...
			<table class="full">						
				<tr>
					<td colspan="2">
						...
						<xsl:text>&#x20;</xsl:text>
						<xsl:value-of select="$i18n.SendInvitationImmediately"/>
					</td>
				</tr>					
			</table>
			...
			<p class="error">
				<xsl:choose>
					...
					<xsl:when test="validationErrorType='InvalidFormat'">
						<xsl:value-of select="$i18n.ValidationError.InvalidFieldValueFormat"/>
					</xsl:when>		
					<xsl:otherwise>
						<xsl:value-of select="$i18n.ValidationError.UnknownFieldError"/>
					</xsl:otherwise>
				</xsl:choose>
