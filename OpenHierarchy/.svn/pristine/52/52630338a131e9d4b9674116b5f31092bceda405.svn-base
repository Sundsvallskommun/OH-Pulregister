...
public class MySQLForegroundModuleDAO extends MySQLModuleDAO<SimpleForegroundModuleDescriptor> implements ForegroundModuleDAO {
	...
	@Override
	public boolean hasModules(Integer sectionID) throws SQLException {
		...
		try {
			...
			BooleanQuery query = new BooleanQuery(dataSource, "SELECT 1 FROM openhierarchy_foreground_modules WHERE sectionID = ? LIMIT 1");
