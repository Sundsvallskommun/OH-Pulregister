...
public class SectionCache {
	...
	public SectionInterface cache(SimpleSectionDescriptor simpleSectionDescriptor) {
		...
		try {
			...
			sectionInstance.cacheModulesAndSections();
	...
	public void cacheSections(ThreadPoolTaskGroupHandler<TaskGroup> threadPoolTaskHandler) throws KeyNotCachedException, KeyAlreadyCachedException, SQLException {
			...
			if (sectionBeanList != null && sectionBeanList.size() > 0) {
				...
				for (SimpleSectionDescriptor simpleSectionDescriptor : sectionBeanList) {
					...
					if (this.sectionInstanceMap.containsKey(simpleSectionDescriptor)) {
						...
						try {
							this.cache(simpleSectionDescriptor, threadPoolTaskHandler);
	...
	public SectionInterface cache(SimpleSectionDescriptor simpleSectionDescriptor, ThreadPoolTaskGroupHandler<TaskGroup> threadPoolTaskHandler) {
		...
		try {
			...
			sectionInstance.cacheModulesAndSections(threadPoolTaskHandler);
