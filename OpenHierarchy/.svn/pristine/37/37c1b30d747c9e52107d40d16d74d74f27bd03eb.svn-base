...
import se.unlogic.hierarchy.core.tasks.ModuleStartupTask;
...
public class ForegroundModuleCache extends BaseModuleCache<ForegroundModuleDescriptor, ForegroundModule, ForegroundModuleCacheListener> {
			...
			if (moduleDescriptorBeanList != null && moduleDescriptorBeanList.size() > 0) {
				...
				for (ForegroundModuleDescriptor moduleDescriptor : moduleDescriptorBeanList) {
					if (this.instanceMap.containsKey(moduleDescriptor)) {
						...
						// Cache new module
						log.info("Adding module " + moduleDescriptor + " to cache in section " + this.sectionInterface.getSectionDescriptor() + "...");
						try {
	...
	@Override
	public void cacheModules(BlockingQueue<Runnable> taskQueue) throws KeyNotCachedException, KeyAlreadyCachedException, SQLException {

		w.lock();
		try {
			// Get all enabled modules from the database
			List<? extends ForegroundModuleDescriptor> moduleDescriptors = moduleDAO.getEnabledModules(this.sectionInterface.getSectionDescriptor().getSectionID());

			if (moduleDescriptors != null) {

				for (final ForegroundModuleDescriptor moduleDescriptor : moduleDescriptors) {

					if (!this.instanceMap.containsKey(moduleDescriptor)) {

						taskQueue.add(new ModuleStartupTask<ForegroundModuleDescriptor, ForegroundModuleCache>(moduleDescriptor, this, sectionInterface.getSectionDescriptor()));
					}
				}
			}
	...
	protected void removeFromAliasMap(ForegroundModuleDescriptor descriptor) {
		...
		aliasMap.values().removeAll(Collections.singletonList(descriptor));
	}
	
	public void addModuleAlias(ForegroundModuleDescriptor descriptor, String alias) throws ModuleNotCachedException, CachePreconditionException{
		...
		try {
			...
			ForegroundModuleDescriptor conflictingDescriptor = aliasMap.get(alias);
			...
			for(ForegroundModuleDescriptor moduleDescriptor : cacheInProgressSet){
				...
				if(moduleDescriptor.getAlias().equals(alias) && !moduleDescriptor.equals(descriptor)){
			...
			this.aliasMap.put(alias, descriptor);
	...
	public boolean removeModuleAlias(ForegroundModuleDescriptor descriptor, String alias) throws ModuleNotCachedException{
		...
		try {
			...
			ModuleDescriptor cachedDescriptor = aliasMap.get(alias);
			
			if(cachedDescriptor != null && descriptor.equals(cachedDescriptor)){
	...
	public List<String> getModuleAliases(ForegroundModuleDescriptor descriptor) throws ModuleNotCachedException{
		...
		try {
			...
			List<String> aliases = new ArrayList<String>(this.aliasMap.size());
			
			for(Entry<String,ForegroundModuleDescriptor> entry : this.aliasMap.entrySet()){
	...
	@Override
	protected void checkCachePreconditions(ForegroundModuleDescriptor descriptor) throws AliasCollisonException, InvalidModuleAliasException, InvalidModuleNameException, InvalidModuleDescriptionException  {
	...
	private void validateDescriptor(ForegroundModuleDescriptor descriptor) throws InvalidModuleAliasException, InvalidModuleNameException, InvalidModuleDescriptionException {
