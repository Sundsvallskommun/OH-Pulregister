...
Ext.define('OpenEMap.action.Action', {
    ...
    constructor: function(config) {
        ...
        if (config.minScale || config.maxScale) {
            ...
            if (!config.maxScale)  {
                config.maxScale = 9.9999999999999E13;
            }
            
            var onZoomend = function() {
                    if (map.getScale() >= config.maxScale || map.getScale() <= config.minScale) {
                        this.disable();
                    } else {
                        this.enable();
                    }
...
Ext.define('OpenEMap.view.DetailReportResults', {
    ...
    initComponent: function() {
        ...
        this.tpl = new Ext.XTemplate('<h3>' + this.fbet + '</h3>', '<h4>' + this.aktbet + '</h4>', '<tpl for=".">', '<div style="margin-bottom: 10px;" class="thumb-wrap">', '<h4>{COUNT}. {DESCRIPTIO}</h4>', '<p>{REMARK}</p>', '</div>', '</tpl>');
...
Ext.define('OpenEMap.action.DrawGeometry', {
    ...
    isText: function(feature) {
        if (feature) {
            ...
            if (isPoint) {
                var isText = feature.attributes && feature.attributes.type && feature.attributes.type === 'label';
                return isText;
            }
        }
        return false;
    ...
    constructor: function(config) {
        ...
        if (!config.tooltip) {
            config.tooltip = config.geometry === 'Polygon' ? 'Rita område' : config.geometry === 'Path' ? 'Rita linje' : config.geometry === 'Point' ? 'Rita punkt' : 'Rita geometri';
            config.iconCls = config.geometry === 'Polygon' ? 'action-drawpolygon' : config.geometry === 'Path' ? 'action-drawline' : config.geometry === 'Point' ? 'action-drawpoint' : 'action-drawgeometry';
...
Ext.define('OpenEMap.view.ObjectConfig', {
    ...
    createAttributeField: function(feature, key) {
        ...
        if (metadata && metadata[key] && metadata[key].hidden)  {
            return;
        }
...
Ext.define('OpenEMap.action.DrawObject', {
    ...
    toggle: function(pressed) {
        if (pressed)  {
            this.objectConfigView.setConfig(this.objectConfig);
        }
...
Ext.define('OpenEMap.view.IdentifyResults', {
    ...
    addResult: function(features, layer) {
        ...
        var getMainAttribute = function(feature, layer) {
                ...
                if (str === '') {
                    str = feature.attributes[Object.keys(feature.attributes)[0]];
                }
                // If no main attribute is specified use the first
                str.trim();
                return str;
            };
...
Ext.define('OpenEMap.action.ModifyGeometry', {
    ...
    constructor: function(config) {
        ...
        if (config.reshape === undefined)  {
            config.reshape = true;
        }
        
        var mode = 0;
        if (config.drag)  {
            mode = mode | OpenLayers.Control.ModifyFeature.DRAG;
        }
        
        if (config.rotate)  {
            mode = mode | OpenLayers.Control.ModifyFeature.ROTATE;
        }
        
        if (config.resize)  {
            mode = mode | OpenLayers.Control.ModifyFeature.RESIZE;
        }
        
        if (config.reshape)  {
            mode = mode | OpenLayers.Control.ModifyFeature.RESHAPE;
        }
            ...
            listeners: {
                beforeclose: function() {
                    ...
                    map.events.triggerEvent("popupfeatureunselected", {
                        ...
                        featureid: this.feature.attributes[this.feature.layer.idAttribute]
                    });
                }
            }
        });
        ...
        var Click = OpenLayers.Class(OpenLayers.Control, {
                ...
                onClick: function(evt) {
                    ...
                    var popupIdentify = function(popupLayer) {
                            ...
                            if (popupLayer.popup) {
                                // Remove popup window
                                popupLayer.popup.forEach(function(p) {
                                    p.destroy();
                                    p = null;
                                });
                                popupLayer.popup = [];
                            ...
                            popupLayer.features.forEach(function(feature) {
                                ...
                                map.events.triggerEvent("popupfeatureunselected", {
                                    layer: popupLayer,
                                    featureid: feature.attributes[popupLayer.idAttribute]
                                });
                            });
                            var popupFeature = function(feature) {
                                    if (feature.geometry.intersects(bounds.toGeometry())) {
                                        ...
                                        if ((typeof popupLayer.popupTextAttribute !== 'undefined') && (popupLayer.popupTextAttribute !== null)) {
                                            ...
                                            // Create popup 
                                            var popup = new OpenEMap.view.PopupResults({
                                                    mapPanel: mapPanel,
                                                    location: clkFeature,
                                                    popupText: popupText,
                                                    feature: feature,
                                        ...
                                        map.events.triggerEvent("popupfeatureselected", {
                                            layer: popupLayer,
                                            featureid: feature.attributes[popupLayer.idAttribute]
                                        });
                                        return true;
...
Ext.define('OpenEMap.action.Print', {
    ...
    constructor: function(config) {
        ...
        printProvider.customParams = {
            attribution: config.mapPanel.config.attribution.trim(),
            mapTitle: ''
        };
...
Ext.define('OpenEMap.view.BaseLayers', {
    ...
    constructor: function(config) {
        var mapPanel = config.mapPanel;
        var map = mapPanel.map;
        var baseLayers = mapPanel.map.layers.filter(function(layer) {
                return layer.isBaseLayer;
            });
        var createButton = function(layer) {
                var cls;
                if (layer == baseLayers[baseLayers.length - 1])  {
                    cls = 'oep-tools-last';
                }
...
*/
    ...
    parseStyle: function(style) {
        ...
        if (style) {
            if (style["default"]) {
                ...
                defaultStyle = new OpenLayers.Style(null, {
                    rules: [
                        new OpenLayers.Rule({
                            symbolizer: defaultStyle
                        })
                    ]
                });
            }
            if (style["select"]) {
                ...
                selectStyle = new OpenLayers.Style(null, {
                    rules: [
                        new OpenLayers.Rule({
                            symbolizer: selectStyle
                        })
                    ]
                });
            }
            if (style["temporary"]) {
                ...
                temporaryStyle = new OpenLayers.Style(null, {
                    rules: [
                        new OpenLayers.Rule({
                            symbolizer: temporaryStyle
                        })
                    ]
                });
            }
            if (style["labelLength"]) {
                ...
                measureStyle = new OpenLayers.Style(null, {
                    rules: [
                        new OpenLayers.Rule({
                            symbolizer: measureStyle
                        })
                    ]
                });
            }
    ...
    initDefaultLayers: function(config) {
        ...
        this.drawLayer.styleMap.styles['default'].addRules([
            labelRule
        ]);
        ...
        var createStyleMap = function(name) {
                return new OpenLayers.StyleMap({
                    'default': OpenLayers.Util.applyDefaults(null, defaultStyles[name])
                });
            };
            ...
            'beforequery': function(queryPlan) {
                if (queryPlan.query.length < this.minChars) {
                    ...
                    if (this.store.loading && this.store.lastOperation) {
                        ...
                        for (var id in requests) if (requests.hasOwnProperty(id) && requests[id].options == this.store.lastOperation.request) {
    ...
    initComponent: function() {
        ...
        this.store = Ext.create('Ext.data.Store', {
            ...
            fields: [
                ...
                {
                    name: 'hit',
                    mapping: '_source.properties.TEXT'
                },
                // Could be AKT="2281K-NJU-268" or PLANNUMMER="S5086"
                {
                    name: 'geometry',
                    mapping: '_source.geometry'
                }
            ]
        });
        this.store.on('load', function(store, records, successful, eOpts) {
...
Ext.define('OpenEMap.view.SearchFastighet', {
    ...
    initComponent: function() {
        ...
        var selectionModel = Ext.create('GeoExt.selection.FeatureModel', {
...
Ext.define('OpenEMap.data.GroupedLayerTree', {
    ...
    maxLayerIndex: 1000,
    listeners: {
        beforeinsert: function(store, node, refNode, eOpts) {
            return this.onBeforeInsert(store, node, refNode);
        },
        beforeappend: function(store, node, eOpts) {
            return this.onBeforeAppend(store, node);
        },
        insert: function(store, node, refNode, eOpts) {
            this.onInsertAndAppend(node);
        },
        append: function(store, node, index, eOpts) {
            this.onInsertAndAppend(node);
        },
        remove: function(store, node, isMove, eOpts) {
            this.onRemove(store, node, isMove);
        }
    },
    constructor: function(config) {
    ...
    */
    getLayerConfiguration: function(includeLayerRef) {
        ...
        function configAddLayer(node, includeLayerRef) {
            ...
            if (!layerCfg.wms || Object.keys(layerCfg.wms).length === 0)  {
                layerCfg.wms = undefined;
            }
            
            if (!layerCfg.wfs || Object.keys(layerCfg.wfs).length === 0)  {
                layerCfg.wfs = undefined;
            }
...
Ext.define('OpenEMap.view.layer.Tree', {
            ...
            function checkParent(n, checked) {
                ...
                if (parent.get("checked") != n.get("checked")) {
                    if (checked) {
                        ...
                        if (!parent.isRoot() && !parent.childNodes.some(function(node) {
    ...
    getBaseLayersConfiguration: function() {
        ...
        var baseLayers = this.mapPanel.map.layers.filter(function(layer) {
                return layer.isBaseLayer;
            });
...
Ext.define('OpenEMap.data.SavedMapConfigs', {
    ...
    constructor: function(config) {
        this.proxy = {
            type: 'rest',
            appendId: true,
            url: ((OpenEMap && OpenEMap.wsUrls && OpenEMap.wsUrls.basePath) ? OpenEMap.wsUrls.basePath : '') + ((OpenEMap && OpenEMap.wsUrls && OpenEMap.wsUrls.adminconfigs) ? OpenEMap.wsUrls.adminconfigs : '') + '/configlist',
            reader: {
                type: 'json',
                root: 'configs'
            },
        ...
        this.columns = [
            {
                ...
                listeners: {
                    click: function(grid, rowIndex, cellIndex, column, e, record, tr) {
                        var requestUrl = ((OpenEMap && OpenEMap.wsUrls && OpenEMap.wsUrls.basePath) ? OpenEMap.wsUrls.basePath : '') + ((OpenEMap && OpenEMap.wsUrls && OpenEMap.wsUrls.configs) ? OpenEMap.wsUrls.configs : '') + '/config/' + record.get('configId');
                        Ext.Ajax.request({
                            scope: this,
                            url: requestUrl,
                            success: function(response) {
                ...
                handler: function(grid, rowIndex, cellIndex, column, e, record, tr) {
                    ...
                    Ext.MessageBox.confirm('Ta bort', 'Vill du verkligen ta bort konfigurationen?', function(btn) {
                        if (btn === 'yes') {
                            ...
                            grid.panel.dataHandler.deleteConfiguration(record.get('configId'), {
...
Ext.define('OpenEMap.view.layer.Advanced', {
    extend: Ext.container.Container,
    // For some reason needed to use drag drop
    //	resizable: true,
    //	resizeHandles: "s",
    layout: {
        type: 'vbox',
            ...
            buttons: [
                {
                    ...
                    handler: function() {
                        if (me.orginalConfig) {
                            ...
                            Ext.MessageBox.prompt('Namn', 'Ange ett namn:', function(btn, text) {
...
Ext.define('OpenEMap.Gui', {
    ...
    constructor: function(config) {
        ...
        this.addEvents('layerControlLoaded', 'addLayerControlLoaded');
        // GUI defaults
        if (this.gui === undefined) {
            this.gui = {};
        }
        if (this.gui.map === undefined) {
            this.gui.map = false;
        }
        if (this.gui.map === {}) {
            this.gui.map = false;
        }
        if (this.gui.layerControl === undefined) {
            this.gui.layerControl = {};
        }
        if (this.gui.addLayerControl === undefined) {
        ...
        if (this.zoomTools && !this.gui.zoomTools.renderTo)  {
            items.push(this.zoomTools);
        }
        
        if (this.objectConfig && !this.gui.objectConfig.renderTo)  {
            items.push(this.objectConfig);
        }
        
        if (this.scalebar && !this.gui.scalebar.renderTo)  {
            items.push(this.scalebar);
        }
        
        if (this.showCoordinate && !this.gui.showCoordinate.renderTo)  {
            items.push(this.showCoordinate);
        }
        
        if (this.searchCoordinate && !this.gui.searchCoordinate.renderTo)  {
            items.push(this.searchCoordinate);
        }
        
        if (this.layerControl && !this.gui.layerControl.renderTo)  {
            items.push(this.layerControl);
        }
        
        if (this.addLayerControl && !this.gui.addLayerControl.renderTo)  {
            items.push(this.addLayerControl);
        }
        
        //        if (this.rightPanel && !this.gui.rightPanel.renderTo) items.push(this.rightPanel);
        if (this.baseLayers && !this.gui.baseLayers.renderTo)  {
            items.push(this.baseLayers);
        }
        
        if (this.toolbar && !this.gui.toolbar.renderTo)  {
    ...
    destroy: function() {
        ...
        if (this.searchFastighet)  {
            this.searchFastighet.destroy();
        }
        
        if (this.searchCoordinate)  {
            this.searchCoordinate.destroy();
        }
        
        if (this.showCoordinate)  {
            this.showCoordinate.destroy();
        }
        ...
        if (this.objectConfig)  {
            this.objectConfig.destroy();
        }
        
        if (this.objectConfigWindow)  {
            this.objectConfigWindow.destroy();
        }
    ...
    createToolbar: function() {
        ...
        var createAction = function(type) {
                ...
                if ((Ext.isObject(type) && Ext.Object.getSize(type) > 0) || (!Ext.isObject(type) && !Ext.isEmpty(type))) {
                    if (type === this.config.tools[this.config.tools.length - 1])  {
                        cls = 'oep-tools-last';
                    }
    ...
    createLayerControl: function() {
        ...
        if (!this.gui.layerControl.y) {
            this.gui.layerControl.y = 20;
        }
        if (!this.gui.layerControl.right) {
            this.gui.layerControl.right = 20;
        }
        if (!this.gui.layerControl.style) {
            this.gui.layerControl.style = 'right: ' + this.gui.layerControl.right + 'px';
        }
        if (!this.gui.layerControl.width) {
            this.gui.layerControl.width = 300;
        }
        if (!this.gui.layerControl.maxHeight) {
            this.gui.layerControl.maxHeight = window.innerHeight - this.gui.layerControl.y - 60;
        }
        if (!this.gui.addLayerControl.y) {
            this.gui.addLayerControl.y = this.gui.layerControl.y;
        }
        if (!this.gui.addLayerControl.right) {
            this.gui.addLayerControl.right = this.gui.layerControl.width + this.gui.layerControl.right;
        }
        if (!this.gui.addLayerControl.style) {
            this.gui.addLayerControl.style = 'right: ' + this.gui.addLayerControl.right + 'px';
        }
        if (!this.gui.addLayerControl.width) {
            this.gui.addLayerControl.width = 300;
        }
        if (!this.gui.addLayerControl.maxHeight) {
            this.gui.addLayerControl.maxHeight = window.innerHeight - this.gui.addLayerControl.y - 56;
        }
...
Ext.define('OpenEMap.config.Parser', {
    ...
    parse: function(config) {
        ...
        options.layers = layers.//.filter(this.isWMSLayer)
        map(function(layer) {
            return layer.layer;
        });
        options.layers = options.layers.filter(function(layer) {
            return layer;
        });
    ...
    sortBaseLayers: function(layers) {
        var baseLayers = layers.filter(function(layer) {
                return (layer.layer && layer.layer.isBaseLayer);
            });
        var overlays = layers.filter(function(layer) {
                return !(layer.layer && layer.layer.isBaseLayer);
            });
        var sorted = false;
...
*/
        // filter out groups
        var groups = layers.filter(function(layer) {
                return layer.layers;
            }).map(function(layer) {
                return layer.layers;
            });
    ...
    extractWFS: function(layers) {
        layers = layers.filter(function(layer) {
            return (layer.wfs && layer.wfs.url);
        });
        return layers;
    },
    /**
    ...
    extractWMS: function(layers) {
        layers = layers.filter(function(layer) {
            return (layer.wms && layer.wms.url);
        });
        return layers;
    },
    /**
    ...
    createLayer: function(layer) {
        if (layer.wms) {
            ...
            return new OpenLayers.Layer.Bing(Ext.apply({
                name: layer.name
            }, layer.bing.options));
        } else {
...
OpenLayers.Control.ModifyFeature = OpenLayers.Class(OpenLayers.Control, {
    ...
    initialize: function(layer, options) {
        ...
        var dragOptions = {
                ...
                setEvent: function(evt) {
                    ...
                    _self._lastVertex = feature ? feature.layer.getFeatureFromEvent(evt) : null;
    ...
    createVirtualVertex: function(point1, point2) {
        ...
        var point = new OpenLayers.Feature.Vector(new OpenLayers.Geometry.Point(x, y), null, this.virtualStyle);
        point._sketch = true;
        return point;
    },
    /**
    ...
    activate: function() {
        if (OpenLayers.Control.prototype.activate.apply(this, arguments)) {
            ...
            return this.handlers.keyboard.activate() && this.handlers.drag.activate();
        }
        return false;
    },
    /**
    ...
    selectFeature: function(feature) {
        if (this.feature === feature || (this.geometryTypes && OpenLayers.Util.indexOf(this.geometryTypes, feature.geometry.CLASS_NAME) == -1)) {
            return;
        }
    ...
    dragStart: function(feature) {
        ...
        if (this.feature && (feature._sketch || isPoint && feature === this.feature)) {
...
OpenLayers.Control.DynamicMeasure = OpenLayers.Class(OpenLayers.Control.Measure, {
    ...
    initialize: function(handler, options) {
        ...
        if (options.drawingLayer) {
            ...
            if (sketchStyle) {
                options.handlerOptions.layerOptions = OpenLayers.Util.applyDefaults(options.handlerOptions.layerOptions, {
        ...
        if (!options.handlerOptions.layerOptions || !options.handlerOptions.layerOptions.styleMap) {
            ...
            var pointStyle = OpenLayers.Util.applyDefaults(optionsStyles.Point, defaultStyles.Point);
            var lineStyle = OpenLayers.Util.applyDefaults(optionsStyles.Line, defaultStyles.Line);
            var polygonStyle = OpenLayers.Util.applyDefaults(optionsStyles.Polygon, defaultStyles.Polygon);
            var style = new OpenLayers.Style(null, {
                    rules: [
                        new OpenLayers.Rule({
                            symbolizer: {
            ...
            options.handlerOptions.layerOptions.styleMap = new OpenLayers.StyleMap({
                'default': style
            });
        }
        // * positions option
            ...
            var _create = function(styleName, initialOptions) {
                    ...
                    if (!options.styleMap) {
                        ...
                        options.styleMap = new OpenLayers.StyleMap({
                            'default': OpenLayers.Util.applyDefaults(style, _defaultStyles[styleName])
                        });
                    }
    ...
    callbackDone: function(geometry) {
        var feature = new OpenLayers.Feature.Vector(geometry);
        this.mapPanel.measureLayer.addFeatures([
            feature.clone()
        ]);
    ...
    drawFeature: function(geometry) {
        var feature = new OpenLayers.Feature.Vector(geometry);
        var proceed = this.drawingLayer.events.triggerEvent('sketchcomplete', {
                feature: feature
            });
        if (proceed !== false) {
            feature.state = OpenLayers.State.INSERT;
    ...
    callbackModify: function(point, feature, drawing) {
        ...
        if (this.isArea) {
            if (this.layerArea) {
                this.showLabel(this.layerArea, 1, 0, this.getBestArea(feature.geometry), positionGet[positions['labelArea']](), 1);
            }
            if (this.showLabelSegment(1, 0, [
                line.components[0],
                line.components[currIndex]
            ])) {
                dynamicObj.countSegments++;
            }
        }
    ...
    showLabelSegment: function(labelsNumber, fromIndex, _points) {
        ...
        var from = points[0],
            ...
            segmentLength = this.getBestLength(new OpenLayers.Geometry.LineString(points)),
        ...
        if (layerHeading && segmentLength[0] > 0) {
            ...
            created = created || this.showLabel(layerHeading, labelsNumber, fromIndex, [
                bearing,
                '°'
            ], positionGet[positions['labelHeading']](), this.maxHeadings);
        }
        return created;
    },
    /**
    ...
    showLabel: function(layer, labelsNumber, fromIndex, measure, xy, maxSegments) {
        ...
        if (features.length < labelsNumber) {
            ...
            featureLabel = new OpenLayers.Feature.Vector(new OpenLayers.Geometry.Point(xy[0], xy[1]), {
                from: fromIndex
            });
...
Ext.define('OpenEMap.Client', {
    ...
    addPopupLayer: function(geojson, layername, idAttribute, popupTextAttribute, popupTextPrefix, popupTextSuffix, popupTitleAttribute, stylemap, epsg, zoomToBounds) {
        ...
        var popupLayer = new OpenLayers.Layer.Vector(layername, {
                renderers: renderer,
                idAttribute: idAttribute,
                popupTextAttribute: popupTextAttribute,
                popupTextPrefix: popupTextPrefix,
                popupTextSuffix: popupTextSuffix,
                popupTitleAttribute: popupTitleAttribute
            });
        if (!popupLayer) {
    ...
    showPopupFeaturePopup: function(popupLayer, feature) {
        ...
        if (popupLayer.popup) {
            // Remove any popup window
            popupLayer.popup.forEach(function(item) {
                item.destroy();
            });
        }
        // get text to populate popup 
        ...
        if (features) {
            ...
            if (features.length == 1) {
                ...
                popupLayers.forEach(function(popupLayer) {
                    popupLayer.features.forEach(function(feature) {
                        if (feature.renderIntent == 'select') {
                            ...
                            feature.layer.drawFeature(feature);
                            // Fire action "popupfeatureunselected" on the feature including layer and featureid
                            feature.layer.map.events.triggerEvent("popupfeatureunselected", {
                                layer: popupLayer,
                                featureid: feature.attributes[popupLayer.idAttribute]
                            });
                        }
                ...
                if (center) {
                    ...
                    features[0].layer.map.setCenter([
                        centerPoint.x,
                        centerPoint.y
                    ]);
                }
                ...
                // Fire action "popupfeatureselected" on the feature including layer and featureid
                features[0].layer.map.events.triggerEvent("popupfeatureselected", {
                    layer: popupLayer,
                    featureid: features[0].attributes[popupLayer.idAttribute]
                });
            } else {
