...
Ext.define('GeoExt.Action', {
    ...
    onCtrlActivate: function() {
        ...
        if (ctrl.type == OpenLayers.Control.TYPE_BUTTON) {
            ...
            // deal with buttons
            this.safeCallEach("toggle", [
                true
            ]);
            // deal with check items
            this.safeCallEach("setChecked", [
                true
            ]);
        }
    },
...
Ext.define('GeoExt.FeatureRenderer', {
    ...
    initComponent: function() {
        ...
        Ext.applyIf(this, {
            ...
            polygonFeature: new OpenLayers.Feature.Vector(new OpenLayers.Geometry.Polygon([
    ...
    assignRenderer: function() {
        for (var i = 0,
            len = this.renderers.length; i < len; ++i) {
    ...
    drawFeature: function() {
        ...
        for (var i = 0,
            len = this.symbolizers.length; i < len; ++i) {
            symbolizer = this.symbolizers[i];
            feature = this.feature;
            if (symbolizer instanceof OpenLayers.Symbolizer) {
                symbolizer = symbolizer.clone();
                if (OpenLayers.Symbolizer.Text && symbolizer instanceof OpenLayers.Symbolizer.Text && symbolizer.graphic === false) {
        ...
        toFilter: function(form, logicalOp, wildcard) {
            ...
            for (var prop in values) {
                ...
                var value = values[prop],
                    type;
                if (s.length > 1 && (type = FILTER_MAP[s[1]]) !== undefined) {
                    prop = s[0];
                } else {
...
Ext.define('GeoExt.LegendImage', {
    ...
    setUrl: function(url) {
        ...
        if (el) {
            ...
            el.on("load", this.onImageLoad, this, {
                single: true
            });
...
Ext.define('GeoExt.data.reader.Attribute', {
    ...
    readRecords: function(data) {
        ...
        var attr, values, name, record, ignore, value, field,
            records = [];
        for (var i = 0,
...
Ext.define('GeoExt.data.FeatureStore', {
    ...
    bind: function(layer, options) {
        ...
        if (initDir & GeoExt.data.FeatureStore.LAYER_TO_STORE && layer.features.length > 0) {
...
Ext.define('GeoExt.data.LayerModel', {
    ...
    inheritableStatics: {
        ...
        createFromLayer: function(layer) {
            return this.getProxy().reader.readRecords([
                layer
            ]).records[0];
        }
    },
    ...
    encoders: {
        "layers": {
            ...
            "Markers": function(layer) {
                var features = [];
                for (var i = 0,
                    len = layer.markers.length; i < len; i++) {
        ...
        "legends": {
            "gx_wmslegend": function(legend, scale) {
                ...
                var icons = [];
                for (var i = 1,
                    len = legend.items.getCount(); i < len; ++i) {
...
Ext.define('GeoExt.panel.Map', {
    ...
    warnMapFallThrough: function() {
        Ext.log({
            level: 'warn',
            msg: 'It is recommended to construct a GeoExt.panel.Map with' + ' OpenLayers.Map#fallThrough == true. This way dragging' + ' interactions with floating components (e.g.' + ' Ext.window.Window) on top of the map are smoother.'
        });
    },
    /**
    ...
    fields: [
        ...
        "maxScaleDenominator",
        {
            ...
            convert: function(symbolizers, rec) {
                ...
                symbolizers = (symbolizers instanceof Array) ? symbolizers : [
                    symbolizers
                ];
...
Ext.define('GeoExt.data.reader.WfsCapabilities', {
    ...
    readRecords: function(data) {
        ...
        var records = [];
        for (var i = 0,
            lenI = featureTypes.length; i < lenI; i++) {
            featureType = featureTypes[i];
            if (featureType.name) {
                metadata = {};
                for (var j = 0,
                    lenJ = fields.length; j < lenJ; j++) {
                    field = fields[j];
                ...
                if (this.protocolOptions) {
                    Ext.apply(protocolOptions, this.protocolOptions, protocolDefaults);
                } else {
                ...
                layer = new OpenLayers.Layer.Vector(featureType.title || featureType.name, layerOptions);
                records.push(layer);
            }
        }
        return this.callParent([
        ...
        if (url && layers) {
            ...
            for (var i = 0,
                ...
                if (layer.name) {
                    ...
                    layer = new OpenLayers.Layer.WMS(layer.title || layer.name, url, params, options);
                    records.push(layer);
                }
            }
        }
...
Ext.define('GeoExt.form.action.Search', {
    ...
    run: function() {
        ...
        if (this.clientValidation === false || form.isValid()) {
            ...
            this.form.prevResponse = this.protocol.read(Ext.applyIf({
                filter: f,
    ...
    defaultRenderer: function(value, meta, record) {
        if (value) {
            ...
            if (record) {
                ...
                if (className == "OpenLayers.Geometry.Point" || className == "OpenLayers.Geometry.MultiPoint") {
                    symbolType = "Point";
                } else if (className == "OpenLayers.Geometry.Polygon" || className == "OpenLayers.Geometry.MultiPolygon") {
                    symbolType = "Polygon";
                }
            }
            window.setTimeout(function() {
                ...
                if (ct) {
                    var renderer = Ext.create('GeoExt.FeatureRenderer', {
                            renderTo: ct,
                            symbolizers: value instanceof Array ? value : [
                                value
                            ],
...
Ext.define('GeoExt.panel.PrintMap', {
    ...
    initComponent: function() {
        ...
        if (!(this.printProvider instanceof GeoExt.data.MapfishPrintProvider)) {
            this.printProvider = Ext.create('GeoExt.data.MapfishPrintProvider', this.printProvider);
        }
...
Ext.define('GeoExt.plugins.PrintExtent', {
    ...
    onMapPanelDestroy: function() {
        var map = this.map;
        for (var len = this.pages.length - 1,
            i = len; i >= 0; i--) {
...
Ext.define('GeoExt.plugins.PrintProviderField', {
    ...
    init: function(target) {
        ...
        onCfg[target instanceof Ext.form.ComboBox ? "select" : "change"] = this.onFieldChange;
        target.on(onCfg);
    },
    /**
...
Ext.define('GeoExt.selection.FeatureModel', {
     ...
     */
    getLayers: function() {
        return this.selectControl.layers || [
            this.selectControl.layer
        ];
    },
    /**
...
Ext.define('GeoExt.slider.Tip', {
    ...
    registerThumbListeners: function() {
        var thumb, el;
        for (var i = 0,
            ii = this.slider.thumbs.length; i < ii; ++i) {
...
Ext.define('GeoExt.tree.LayerNode', {
    ...
    onCheckChange: function() {
        ...
        if (checked != node.get('layer').getVisibility()) {
            ...
            } else if (!checked && layer.isBaseLayer && layer.map && layer.map.baseLayer && layer.id == layer.map.baseLayer.id) {
    ...
    onChildMove: function(node, oldParent, newParent, index) {
        ...
        if (parentLoader instanceof me.self && me.store === parentLoader.store) {
            ...
            if (newRecordIndex !== undefined) {
                me.store.insert(newRecordIndex, [
                    record
                ]);
            } else {
                me.store.insert(oldRecordIndex, [
                    record
                ]);
            }
...
Ext.define('GeoExt.tree.Column', {
    ...
    initComponent: function() {
        ...
        this.renderer = function(value, metaData, record, rowIdx, colIdx, store, view) {
            var buf = [
                    parentRenderer.call(this, value, metaData, record, rowIdx, colIdx, store, view)
                ];
...
Ext.define('GeoExt.window.Popup', {
    ...
    onRender: function(ct, position) {
        ...
        var ancDiv = dh.append(this.el.dom, // the context element
            spec);
