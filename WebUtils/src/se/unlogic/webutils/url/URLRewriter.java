...
public class URLRewriter {
	...
	public static boolean containsTag(String text, String tag) {
		...
		for (String attribute : TAG_ATTRIBUTES) {
			...
			if(text.contains(attribute + "=\"" + tag)){
	...
	public static boolean hasAbsoluteLinkUrls(Object bean) {
			...
			for (Field field : fields) {
				...
				if (value != null) {
					...
					if (value instanceof String) {
						...
						if(containsTag((String)value, RELATIVE_URL_MARKER)){
						...
						for (Object object : (Collection<?>) value) {
							...
							hasAbsoluteLinkUrls(object);
						}

					} else {

						hasAbsoluteLinkUrls(value);
					}
				}
			}
